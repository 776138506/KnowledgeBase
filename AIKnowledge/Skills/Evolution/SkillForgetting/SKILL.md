---
name: SkillForgetting
description: 技能遗忘与清理专家，负责根据使用频率识别低频技能（<1 次/月）并建议废弃、根据遗忘时间识别过时技能（>6 个月未使用）并建议遗忘、通过关联保护机制确保删除技能后相关技能不失效、支持行级 CRUD 精细化修改而非简单文件删除、维护技能库精简性和高效性，适用于技能生命周期管理和技能库优化场景
---
